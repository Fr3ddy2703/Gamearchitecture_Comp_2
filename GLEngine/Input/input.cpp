...
void KeyBoardInput::processInput(GLFWwindow* window, std::shared_ptr<Player> player)
{
	...
	if(glfwGetKey(window, GLFW_KEY_W)==GLFW_PRESS)
	{
		...
		{
			player->GetPlayerPos().x += initializer::UseCamera.cameraSpeed * initializer::UseCamera.cameraFront.x * initializer::DeltaTime;
			player->GetPlayerPos().z += initializer::UseCamera.cameraSpeed * initializer::UseCamera.cameraFront.z * initializer::DeltaTime;
			initializer::UseCamera.setPlayerPos(player->GetPlayerPos());
		}
	}
	...
	if(glfwGetKey(window, GLFW_KEY_A)==GLFW_PRESS)
	{
		...
		{
			player->GetPlayerPos() -= (initializer::UseCamera.cameraSpeed * glm::normalize(glm::cross(initializer::UseCamera.cameraFront, initializer::UseCamera.cameraUp))) * initializer::DeltaTime;
			initializer::UseCamera.setPlayerPos(player->GetPlayerPos());
		}
			}
    ...
    {
		initializer::UseCamera.CameraLock = false;
    }
	if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS)
    {
		initializer::UseCamera.CameraLock = true;
    }
}
